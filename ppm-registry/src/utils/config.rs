...
const DB_USER: &str = "ppm_admin";
const DB_PASS: &str = "ppm_admin";

pub type StorageSetup = ppm_core::ppm::registry::v1::storage::StorageBackend;
pub type ServerSetup = ppm_core::ppm::registry::v1::Server;
...
impl ConfigBuilder {
    ...
    pub fn data(&mut self, data_setup: ppm_core::Data) -> &mut Self {
    ...
    pub fn build(self) -> ppm_core::Config {
        ppm_core::Config {
            data: Some(self.data),
            server: Some(self.server),
            storage: Some(ppm_core::Storage { 
...
fn default_local_registry_path() -> String {
    let curr_dir = ppm_core::FileSystem::current_dir()
        .map_err(|err| RegistryError::InvalidConfigSetup(err.to_string())).unwrap();
    String::from(ppm_core::FileSystem::join_paths(curr_dir, REGISTRY_PATH).as_path().to_str().unwrap())
}

fn default_db_setup() -> ppm_core::Data {
    ppm_core::Data {
